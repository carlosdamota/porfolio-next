...
interface ButtonProps {
  ...
  target?: string;
  download?: boolean;
  style?: "primary" | "secondary" | "primary-card" | "secondary-card";
  className?: string;
  disabled?: boolean;
}
...
export const Button = ({
  ...
  disabled = false,
    ...
    const baseStyles = "px-4 sm:px-6 md:px-8 py-2 rounded-xl gap-4 text-sm sm:text-md flex items-center justify-center glass";
    ...
    <Link
      ...
      target={target || ''}
      className={`${baseStyles} ${styles[style || 'primary']} ${className}`}
    >
      {children}
    </Link>
  )
